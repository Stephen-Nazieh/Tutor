...
async function upsertReplayArtifact(input) {
    ...
    await query(`INSERT INTO "SessionReplayArtifact" ("id", "sessionId", "tutorId", "transcript", "summary", "summaryJson", "status", "generatedAt", "createdAt", "updatedAt")
...
async function tick(state) {
    ...
    if (hash === state.lastVttHash)
        return;
    const transcriptText = vttToPlainText(vtt);
    const trimmedTranscript = transcriptText.length > 24_000 ? transcriptText.slice(transcriptText.length - 24_000) : transcriptText;
...
export async function startLiveTranscriptionWorker(input) {
    const existing = await getCache(cacheKey(input.sessionId));
    ...
    if (intervals.has(input.sessionId))
        return;
...
export async function getLiveTranscriptionState(sessionId) {
    return getCache(cacheKey(sessionId));
}
export async function stopLiveTranscriptionWorker(sessionId) {
