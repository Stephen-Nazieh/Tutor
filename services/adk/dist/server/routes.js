...
async function generateWithKimi(messages, options = {}) {
...
router.post('/v1/llm/generate', async (req, res) => {
    ...
    try {
        ...
        res.status(500).json({ error: 'LLM generate failed', message: error.message });
    }
});
router.post('/v1/llm/chat', async (req, res) => {
    ...
    try {
        ...
        res.status(500).json({ error: 'LLM chat failed', message: error.message });
    }
});
...
Respond in JSON format with this structure:
{
  ...
  "confidence": 0.8
    ...
    try {
        ...
        res.status(500).json({ error: 'PCI Master failed', message: error.message });
    }
});
