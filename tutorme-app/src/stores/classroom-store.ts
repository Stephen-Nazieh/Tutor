...
export const useClassroomStore = create<ClassroomState>()(
  immer(set => ({
    ...
    addBreakoutMessage: message =>
      set(state => {
        state.breakoutMessages.push(message)
      }),

    setBreakoutMessages: messages =>
      set(state => {
    ...
    setShowEngagementPanel: show =>
      set(state => {
        state.showEngagementPanel = show
      }),

    setShowSessionTimer: show =>
      set(state => {
...
export const useStrugglingStudents = () =>
  useClassroomStore(state => state.students.filter(s => s.status === 'struggling'))
export const useStrugglingCount = () =>
  useClassroomStore(state => state.students.filter(s => s.status === 'struggling').length)
...
export const useBreakoutAlertCount = () =>
...
export const useCurrentWhiteboardPage = () =>
  useClassroomStore(state => state.whiteboardPages[state.currentPageIndex])
