...
export function StudentLiveWhiteboard({
  ...
  const drawStroke = useCallback(
    (ctx: CanvasRenderingContext2D, stroke: WhiteboardStroke, alpha: number = 1) => {
      if (stroke.type === 'text') {
  ...
  const selectByAttribute = useCallback(
    (attribute: 'color' | 'type' | 'layer') => {
      ...
      myStrokes.forEach(stroke => {
        ...
        if (
          attribute === 'layer' &&
          (stroke.layerId || 'student-personal') === (seed.layerId || 'student-personal')
        )
  ...
  const redrawCanvas = useCallback(() => {
    ...
    remoteSelections.forEach(presence => {
      presence.strokeIds.forEach(id => {
        const stroke =
          myStrokes.find(entry => entry.id === id) || tutorStrokes.find(entry => entry.id === id)
        if (!stroke) return
        const bounds = getStrokeBounds(stroke)
  ...
  const handlePointerUp = (e: React.PointerEvent<HTMLCanvasElement>) => {
    ...
    if (shapeTools.has(tool) && shapeStart && shapePreview) {
      ...
      const fromResolved =
        tool === 'connector'
          ? resolveConnectorEndpoint(shapeStart, myStrokes)
          : { point: shapeStart }
      const toResolved =
        tool === 'connector'
          ? resolveConnectorEndpoint(shapePreview, myStrokes)
          : { point: shapePreview }
  ...
  if (mode === 'floating' && isMinimized) {
    return (
      <div className="fixed bottom-4 right-4 z-50 rounded-lg border bg-white p-3 shadow-lg">
  ...
  return (
    <div
      className={cn(
        'flex flex-col rounded-lg border bg-white shadow-2xl',
        mode === 'floating' ? 'fixed bottom-4 right-4 z-50 w-96' : 'h-full w-full'
      )}
    >
      {/* Header */}
      <div className="flex items-center justify-between rounded-t-lg border-b bg-gray-50 px-3 py-2">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-medium">My Whiteboard</span>
          {isLayerLocked && (
            ...
            <SheetTrigger asChild>
              ...
              >
                {visibleTaskShares.length > 0
                  ? `Live Tasks (${visibleTaskShares.length})`
                  : 'Tasks (0)'}
              </Button>
            </SheetTrigger>
                      ...
                      <div className="mt-2 flex items-center justify-between text-xs">
                        <div className="flex items-center gap-1">
                          <Badge variant="outline">
                            {share.visibleToAll ? 'Class Visible' : 'Private'}
                          </Badge>
                          <Badge variant="secondary">
                            {share.allowCollaborativeWrite ? 'Collaborative' : 'Read only'}
                          </Badge>
                          {share.submissions?.some(
                            submission => submission.userId === myUserId
                          ) && (
      ...
      {visibleTaskShares.length > 0 && (
        <div className="border-b bg-blue-50 px-3 py-2 text-xs text-blue-800">
          Tutor shared {visibleTaskShares.length} live task{visibleTaskShares.length > 1 ? 's' : ''}
          . Click <span className="font-semibold">Live Tasks</span> to open and work on your copy.
        </div>
      )}
      ...
      {tutorStrokes.length > 0 && (
        <div className="flex items-center justify-between border-b bg-blue-50 px-3 py-1.5">
          <span className="text-xs text-blue-700">{tutorStrokes.length} strokes from tutor</span>
          <Button
            variant="ghost"
            size="sm"
      ...
      <div
        ref={containerRef}
        className={cn('p-3', mode === 'floating' ? 'h-64' : 'h-full min-h-[380px]')}
      >
        <div
          className={cn(
          ...
          style={{
            ...
            backgroundSize:
              assignmentOverlay === 'chemistry-structure'
                ? '120px 80px'
                : assignmentOverlay === 'geometry-grid'
                  ? '24px 24px'
                  : '20px 20px',
          }}
        >
