...
interface UseBreakoutSocketReturn {
  ...
  updateVideoState: (
    roomId: string,
    state: { isMuted?: boolean; isVideoOff?: boolean; isScreenSharing?: boolean }
  ) => void
...
export function useBreakoutSocket({
  ...
  const normalizeParticipant = useCallback(
    (p: Partial<BreakoutParticipant> & { id: string; name: string }): BreakoutParticipant => ({
      id: p.id,
  ...
  const normalizeRoom = useCallback(
    (
      room: Partial<BreakoutRoom> & { id: string; name: string; mainRoomId?: string }
    ): BreakoutRoom => ({
      id: room.id,
      name: room.name,
      mainRoomId: room.mainRoomId || sessionId,
      participants: (room.participants || []).map(p =>
        normalizeParticipant(p as Partial<BreakoutParticipant> & { id: string; name: string })
      ),
  ...
  useEffect(() => {
    ...
    const handleRoomsUpdated = (data: {
      rooms: Array<Partial<BreakoutRoom> & { id: string; name: string; mainRoomId?: string }>
    }) => {
    ...
    const handleParticipantJoined = (data: {
      roomId: string
      participant: Partial<BreakoutParticipant> & { id: string; name: string }
    }) => {
      setRooms(prev =>
        prev.map(room => {
    ...
    const handleAlert = (data: {
      roomId: string
      alert: Partial<BreakoutAlert> & { type: BreakoutAlert['type']; message: string }
    }) => {
    ...
    const handleMessage = (
      ...
      const normalizedMessage: BreakoutMessage =
        'content' in data
          ? data
          : {
              id: data.id,
    ...
    const handleMetricsUpdate = (data: {
      roomId: string
      metrics: Partial<BreakoutRoom['metrics']>
    }) => {
      setRooms(prev =>
        prev.map(room => {
          if (room.id === data.roomId) {
  ...
  const sendMessage = useCallback(
    (roomId: string, content: string, type?: 'text' | 'question') => {
  ...
  const broadcastMessage = useCallback(
    (message: string, target: 'all' | 'specific', roomIds?: string[]) => {
