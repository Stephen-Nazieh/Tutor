...
export function useSmartGrouping({ students, config }: UseSmartGroupingProps) {
  const generateSmartGrouping = useCallback((): SmartGroupingSuggestion | undefined => {
    ...
    const unclassified = students.filter(s => !s.skillLevel && s.recentPerformance === undefined)
    ...
    switch (distributionMode) {
      case 'skill_based': {
        ...
        if (intermediate.length > 0 && groups.length < roomCount) {
          for (
            let i = 0;
            i < Math.min(roomCount - groups.length, Math.ceil(intermediate.length / 4));
            i++
          ) {
            const startIdx = i * 4
        ...
        if (advanced.length > 0 && groups.length < roomCount) {
          for (
            let i = 0;
            i < Math.min(roomCount - groups.length, Math.ceil(advanced.length / 4));
            i++
          ) {
            const startIdx = i * 4
      ...
      case 'random': {
        ...
        for (let i = 0; i < roomCount; i++) {
          ...
          const members = shuffled
            .slice(startIdx, startIdx + config.participantsPerRoom)
            .map(s => s.id)
