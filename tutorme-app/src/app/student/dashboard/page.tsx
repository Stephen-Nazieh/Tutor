  ...
  const fetchDashboardData = useCallback(async () => {
    ...
    try {
      const responses = await Promise.all([
        fetch('/api/content').catch(() => ({ ok: true, json: () => Promise.resolve({ contents: [] }) })),
        fetch('/api/gamification').catch(() => ({ ok: true, json: () => Promise.resolve({ success: false }) })),
        fetch('/api/gamification/worlds').catch(() => ({ ok: true, json: () => Promise.resolve({ success: false, data: [] }) })),
        fetch('/api/gamification/quests').catch(() => ({ ok: true, json: () => Promise.resolve({ success: false, data: { quests: [] } }) })),
        fetch('/api/recommendations').catch(() => ({ ok: true, json: () => Promise.resolve({ recommendations: [] }) })),
        fetch('/api/classes?limit=3').catch(() => ({ ok: true, json: () => Promise.resolve({ classes: [] }) })),
        fetch('/api/study-groups?limit=3').catch(() => ({ ok: true, json: () => Promise.resolve({ groups: [] }) })),
        fetch('/api/student/subjects').catch(() => ({ ok: true, json: () => Promise.resolve({ subjects: [] }) })),
        fetch('/api/student/reviews').catch(() => ({ ok: true, json: () => Promise.resolve({ success: false, data: null }) }))
      ])
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8" aria-busy={loading}>
        ...
        <div className="space-y-6">
          ...
          <DashboardCalendar
            ...
            upcomingClasses={(data?.classes || []).map((c: any) => ({
              id: c.id,
              title: c.title,
              subject: c.subject,
          ...
          {data?.dailyQuests && data.dailyQuests.length > 0 ? (
            <DailyQuestsWidget
              ...
              completedCount={data.dailyQuests.filter((q: any) => q.completed).length}
              totalXp={data.dailyQuests.reduce((acc: number, q: any) => acc + (q.completed ? q.quest.xpReward : 0), 0)}
              onQuestClick={(quest: any) => !quest.completed && router.push('/student/missions')}
            />
          ) : (
            <Card>
