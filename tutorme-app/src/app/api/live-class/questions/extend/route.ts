...
const ExtendSchema = z.object({
  question: z.object({
    ...
    type: z.enum(['mcq', 'truefalse', 'shortanswer', 'essay', 'multiselect', 'matching', 'fillblank']),
...
export const POST = withAuth(async (req) => {
  ...
  const basePrompt = [
    ...
    question.matchingPairs ? `Matching Pairs: ${question.matchingPairs.map((p) => `${p.left} -> ${p.right}`).join(' | ')}` : '',
