    ...
    try {
      const conditions = [
        ...
        ...(isTemplate !== null && isTemplate !== ''
          ? [eq(whiteboard.isTemplate, isTemplate === 'true')]
          : []),
      ...
      const snapshotByBoard = Object.fromEntries(
        (snapshotCounts as { whiteboardId: string; count: number }[]).map(r => [
          r.whiteboardId,
          r.count,
        ])
      )
...
export const POST = withAuth(
  async (req: NextRequest, session) => {
    ...
    try {
      ...
      return NextResponse.json({ whiteboard: wb ? { ...wb, pages } : null }, { status: 201 })
    } catch (error) {
