/**
 * Unit tests for GET /api/courses/catalog (reference data for course creation).
 */
...
describe('GET /api/courses/catalog', () => {
  ...
  it('returns 200 and catalog entries', async () => {
    ...
    expect(data.courses[0]).toMatchObject({ subject: 'english', name: 'IELTS' })
  })

  it('supports subject filter query param', async () => {
    const req = new Request('http://localhost/api/courses/catalog?subject=english')
    const res = await GET(req as NextRequest)
