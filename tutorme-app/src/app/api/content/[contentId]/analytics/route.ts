...
export const GET = withAuth(
  async (req, session, context) => {
    ...
    const eventsByStudent = await drizzleDb
      ...
      .where(
        and(eq(videoWatchEvent.contentId, contentId), eq(videoWatchEvent.studentId, forStudentId))
      )
    ...
    const completionPercent =
      durationSec > 0 ? Math.min(100, Math.round((totalWatchSeconds / durationSec) * 100)) : 0
    const lastPosition = eventsByStudent.length
      ? Math.max(...eventsByStudent.map(e => e.videoSeconds))
      : 0
