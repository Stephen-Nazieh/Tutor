...
export async function GET(req: NextRequest) {
  ...
  const medianScore =
    totalStudents > 0 ? (sorted.length % 2 ? sorted[mid] : (sorted[mid - 1] + sorted[mid]) / 2) : 0
  ...
  const avgTime =
    totalStudents > 0 ? Math.round(times.reduce((a, b) => a + b, 0) / totalStudents) : 0
    ...
    questionStatsMap.set(q.id ?? '', {
      ...
      maxPoints:
        ...
        (Array.isArray(q.rubric)
          ? q.rubric.reduce((s: number, r: { points?: number }) => s + (r.points || 0), 0)
          : 0) ??
        1,
  ...
  const questionStats = [...questionStatsMap.values()].map(stat => {
    const correctRate =
      stat.totalAttempts > 0 ? Math.round((stat.correctCount / stat.totalAttempts) * 1000) / 10 : 0
    ...
    return {
      ...
      correctRate,
      avgPoints:
        stat.totalAttempts > 0 ? Math.round((stat.avgPoints / stat.totalAttempts) * 10) / 10 : 0,
      maxPoints: stat.maxPoints,
      avgTimeSec: stat.timeSamples > 0 ? Math.round(stat.avgTimeSec / stat.timeSamples) : null,
