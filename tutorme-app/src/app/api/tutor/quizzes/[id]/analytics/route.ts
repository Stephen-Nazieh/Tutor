...
export const GET = withAuth(
  async (req: NextRequest, session, context) => {
    ...
    completedAttempts.forEach(attempt => {
      ...
      questionResults.forEach((result: any) => {
        ...
        if (!result.correct && result.selectedAnswer && typeof result.selectedAnswer === 'string') {
          stats.wrongAnswers[result.selectedAnswer] =
            (stats.wrongAnswers[result.selectedAnswer] || 0) + 1
        }
      })
    })
    ...
    completedAttempts.forEach(attempt => {
      ...
      const percentage = (attempt.score / attempt.maxScore) * 100
      const attemptWithStudent = attempt as typeof attempt & {
      ...
      if (existing) {
        ...
        if (
          attempt.completedAt &&
          (!existing.lastAttemptAt || attempt.completedAt > existing.lastAttemptAt)
        ) {
