...
import { Upload, File, FileText, Image, X, CheckCircle2, AlertCircle, Loader2 } from 'lucide-react'
    ...
    <div className={className}>
      ...
      >
        ...
        {uploading ? (
          <div className="space-y-2">
            ...
            <Progress value={uploadProgress} className="mx-auto h-1.5 max-w-xs" />
          </div>
        ) : (
          <>
            <Upload className="mx-auto mb-2 h-8 w-8 text-gray-400" />
            <p className="text-sm font-medium text-gray-700">Drop file here or click to browse</p>
            <p className="mt-1 text-xs text-gray-500">PDF, PNG, JPG, DOCX · Max 10MB</p>
          </>
        )}
      </div>
      ...
      {files.length > 0 && (
        <div className="mt-3 space-y-1.5">
          {files.map(f => (
            <div key={f.url} className="flex items-center gap-2 rounded-lg border bg-gray-50 p-2">
              {getFileIcon(f.type)}
      ...
      {files.length >= maxFiles && (
        <p className="mt-2 text-xs text-gray-500">Maximum {maxFiles} files reached</p>
      )}
    </div>
  )
}
