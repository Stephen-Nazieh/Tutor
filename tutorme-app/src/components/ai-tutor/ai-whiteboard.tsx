        ...
        <ScrollArea className="h-64">
          {filteredItems.length === 0 ? (
            <div className="py-8 text-center text-gray-400">
              ...
              <p className="mt-1 text-xs">The AI will add examples, tips, and important notes</p>
            </div>
          ) : (
            <div className="space-y-3">
              {filteredItems.map(item => (
                <div
                  key={item.id}
                  className={`rounded-lg border p-3 text-sm ${getItemStyle(item.type)}`}
                >
