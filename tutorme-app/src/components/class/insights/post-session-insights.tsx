...
export function PostSessionInsights({
  ...
  return (
    <div className="mx-auto w-full max-w-5xl overflow-hidden rounded-xl bg-slate-900">
      {/* Header */}
      <div className="border-b border-slate-700 bg-slate-800 p-6">
          ...
          <div>
            <div className="mb-2 flex items-center gap-2">
              ...
              <span className="text-sm text-slate-400">
                {insights.date.toLocaleDateString()} • {Math.floor(insights.duration / 60)}h{' '}
                {insights.duration % 60}m
              </span>
            </div>
            <h1 className="text-2xl font-bold text-white">{insights.sessionTitle}</h1>
            <p className="mt-1 text-slate-400">Session Insights & Analytics</p>
          </div>
        ...
        <div className="mt-6 grid grid-cols-4 gap-4">
          <StatCard
            ...
            color={
              insights.engagement.averageEngagement >= 70
                ? 'green'
                : insights.engagement.averageEngagement >= 50
                  ? 'yellow'
                  : 'red'
            }
          />
        ...
        <div className="p-6">
          ...
          <TabsContent value="engagement" className="m-0">
            <div className="grid grid-cols-3 gap-6">
              ...
              <Card className="border-slate-700 bg-slate-800">
                <CardHeader>
                  <CardTitle className="text-base text-white">Activity Breakdown</CardTitle>
                </CardHeader>
                <CardContent className="space-y-4">
                  <ActivityBar
                    label="Chat Messages"
                ...
                <CardContent className="space-y-4">
                  {insights.followUp.recommendedAssignments.length > 0 && (
                    <div>
                      ...
                      <div className="space-y-2">
                        {insights.followUp.recommendedAssignments.map((assignment, i) => (
                          <div key={i} className="rounded-lg bg-slate-700/50 p-3">
                            ...
                            <p className="mb-2 text-xs text-slate-400">{assignment.rationale}</p>
                      ...
                      <Button size="sm" className="mt-3 w-full" onClick={onCreateAssignments}>
                ...
                <CardContent>
                  <div className="space-y-3">
                    {insights.followUp.suggestedReviewTopics.map((topic, i) => (
                      <div
                        key={i}
                        className="flex items-center gap-3 rounded-lg border border-yellow-800/50 bg-yellow-900/20 p-3"
                      >
  ...
  return (
    <div
      className={cn(
        ...
        student.flaggedForFollowUp ? 'border border-red-800/50 bg-red-900/20' : 'bg-slate-700/50'
      )}
    >
      ...
      <div className="min-w-0 flex-1">
        <div className="flex items-center gap-2">
          ...
          {student.flaggedForFollowUp && (
            <Badge variant="outline" className="border-red-600 text-[10px] text-red-400">
              Needs Follow-up
            </Badge>
          )}
        </div>
    ...
    <div className="flex gap-4">
      ...
      <div className={cn('flex-1 pb-6', isLast && 'pb-0')}>
        <div className="mb-1 flex items-center gap-2">
          <span className="font-mono text-xs text-slate-500">{formatTime(bookmark.timestamp)}</span>
          <Badge variant="outline" className="text-[10px] capitalize">
            {bookmark.type.replace('_', ' ')}
          </Badge>
        </div>
        <p className="text-sm text-white">{bookmark.description}</p>
        {bookmark.aiNote && <p className="mt-1 text-xs italic text-slate-400">{bookmark.aiNote}</p>}
      </div>
    </div>
  )
}
  ...
  return (
    <div className="flex h-48 items-end gap-1">
      {dataPoints.map((value, i) => (
        <div
          key={i}
          className="flex-1 rounded-t bg-blue-500/50 transition-colors hover:bg-blue-500"
          style={{ height: `${value}%` }}
