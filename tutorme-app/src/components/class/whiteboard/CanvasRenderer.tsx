...
export function CanvasRenderer({
  ...
  const drawBackgroundPattern = useCallback(
    (ctx: CanvasRenderingContext2D, style: string, color: string) => {
      ctx.save()
      ctx.strokeStyle =
        color === '#ffffff' || color === '#fef3c7' ? '#e5e7eb' : 'rgba(255,255,255,0.2)'
      ctx.lineWidth = 1
  ...
  const drawText = useCallback((ctx: CanvasRenderingContext2D, text: TextElement) => {
    ...
    ctx.textAlign = (text.format?.align as CanvasTextAlign) || 'left'
  ...
  const drawSelectionBox = useCallback(
    (ctx: CanvasRenderingContext2D, obj: { type: string; id: string }) => {
      ...
      if (text)
        target = {
          x: text.x - 4,
          y: text.y - (text.fontSize || 16) - 4,
          width: (text.width || 100) + 8,
          height: (text.height || 50) + 8,
        }
      if (!target) return
      ctx.save()
      ...
      ctx.fillRect(target.x + target.width - 4, target.y + target.height - 4, 8, 8)
      ctx.restore()
    },
    [shapes, texts]
  )
  ...
  useEffect(() => {
    ...
    strokes.forEach(stroke =>
      stroke.type === 'eraser'
        ? drawEraserStroke(ctx, stroke.points)
