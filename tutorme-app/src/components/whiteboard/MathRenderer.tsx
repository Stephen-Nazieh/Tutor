...
export function MathRenderer({
  ...
  if (error) {
    return <div className={`text-sm text-red-500 ${className}`}>Error rendering math: {error}</div>
  }
...
export function EquationEditor({ initialLatex = '', onSubmit, onCancel }: EquationEditorProps) {
  ...
  return (
    <div className="w-[500px] max-w-full rounded-lg bg-white p-4 shadow-lg">
      <h3 className="mb-3 text-lg font-semibold">Insert Equation</h3>

      {/* Shortcut buttons */}
      <div className="mb-3">
        <label className="mb-1 block text-xs text-slate-500">Symbols</label>
        <div className="flex flex-wrap gap-1">
          {LATEX_SHORTCUTS.map(({ label, latex: snippet }) => (
            <button
              key={label}
              onClick={() => insertLatex(snippet)}
              className="h-8 w-8 rounded border text-sm transition-colors hover:bg-slate-100"
              title={snippet}
            >
              {label}
            </button>
      ...
      {/* Common equations */}
      <div className="mb-3">
        <label className="mb-1 block text-xs text-slate-500">Templates</label>
        <div className="flex flex-wrap gap-2">
          {commonEquations.map(({ name, latex: eq }) => (
            <button
              key={name}
              onClick={() => setLatex(eq)}
              className="rounded border px-2 py-1 text-xs transition-colors hover:bg-slate-100"
            >
              {name}
            </button>
          ))}
      ...
      {/* LaTeX input */}
      <div className="mb-3">
        <label className="mb-1 block text-xs text-slate-500">LaTeX</label>
        <textarea
          ref={textareaRef}
          value={latex}
      ...
      {/* Preview */}
      <div className="mb-4">
        <label className="mb-1 block text-xs text-slate-500">Preview</label>
        <div className="flex min-h-[60px] w-full items-center justify-center rounded border bg-slate-50 p-3">
          {latex ? (
