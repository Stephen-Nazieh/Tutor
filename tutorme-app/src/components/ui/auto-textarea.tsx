    ...
    ({ className, value, onChange, onKeyDown, ...props }, ref) => {
        ...
        const handleKeyDown = (e: React.KeyboardEvent<HTMLTextAreaElement>) => {
            ...
            if (e.key === 'Enter') {
                ...
                if (match) {
                    ...
                    if (!content.trim()) {
                        const newValue = val.substring(0, selectionStart - currentLine.length) + indent + val.substring(selectionEnd)
                    ...
                    const newValue = val.substring(0, selectionStart) + insertText + val.substring(selectionEnd)
                ...
                if (match && selectionStart === selectionEnd) {
                    ...
                    if (!e.shiftKey) {
                        ...
                        const newValue = val.substring(0, lineStartIdx) + replacedLine + val.substring(selectionEnd)
        ...
        return (
            <div className="relative w-full">
                <Textarea
                    ref={textareaRef}
                    className={cn("w-full transition-all duration-200 pb-8", className)}
                    value={value}
                    onChange={onChange}
                ...
                {(!value || String(value).length < 5) && (
                    <div className="absolute bottom-2 left-3 text-[10px] text-muted-foreground/60 pointer-events-none select-none">
