  ...
  if (hasVoted) {
    return (
      <Card className={cn('p-6 text-center', className)}>
        ...
        <h4 className="mb-2 font-semibold text-gray-900">Thank You!</h4>
        <p className="text-sm text-gray-500">Your response has been recorded.</p>
        {poll.showResults && poll.status === 'active' && (
          <p className="mt-3 text-sm text-blue-600">Results will be shared by your tutor.</p>
        )}
      </Card>
    )
  }
    ...
    <Card className={cn('p-4', className)}>
      ...
      <div className="mb-4">
        <div className="mb-2 flex items-center justify-between">
          <Badge className="border-blue-200 bg-blue-100 text-blue-700">Live Poll</Badge>
          {remainingTime !== null && (
        ...
        <div className="mt-2 flex items-center gap-3 text-sm text-gray-500">
          ...
          {poll.isAnonymous && (
            <span className="rounded bg-gray-100 px-2 py-0.5 text-xs">Anonymous</span>
          )}
          {poll.allowMultiple && (
            <span className="rounded bg-gray-100 px-2 py-0.5 text-xs">Multiple choice</span>
          )}
        </div>
      </div>
...
function renderPollInput({
  ...
  switch (poll.type) {
          ...
          {poll.options.map((option, index) => {
            ...
            return (
              ...
              >
                ...
                <span className={cn('flex-1', isSelected && 'font-medium')}>{option.text}</span>
              </button>
            )
          })}
        </div>
        ...
        <div className="flex justify-center gap-2 py-4">
          {[1, 2, 3, 4, 5].map(value => (
            <button
              ...
              className={cn(
                'h-14 w-14 rounded-xl text-xl font-bold transition-all',
                rating === value
    ...
    case 'word_cloud':
      return (
        <div className="space-y-2">
          <label className="text-sm text-gray-600">Enter a single word or short phrase:</label>
          <Textarea
            value={textAnswer}
            ...
            maxLength={50}
          />
          <p className="text-right text-xs text-gray-400">{textAnswer.length}/50 characters</p>
        </div>
      )
    ...
    case 'short_answer':
      return (
        <div className="space-y-2">
          <label className="text-sm text-gray-600">Enter your answer:</label>
          <Textarea
            value={textAnswer}
            onChange={e => onTextChange(e.target.value)}
            placeholder="Type your answer..."
            ...
            maxLength={500}
          />
          <p className="text-right text-xs text-gray-400">{textAnswer.length}/500 characters</p>
        </div>
      )
