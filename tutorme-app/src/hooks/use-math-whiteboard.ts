...
export function useMathWhiteboard({
  ...
  useEffect(() => {
    ...
    const connect = async () => {
      ...
      const handlePresence = (data: { sessionId: string; participants: MathWBParticipant[] }) => {
      ...
      const handleElementUpdated = (data: {
        ...
        setElements(prev =>
          prev.map(el =>
            el.id === data.elementId ? { ...el, ...data.changes, version: data.version } : el
          )
        )
      }
      ...
      const handleLockChanged = (data: { sessionId: string; locked: boolean; by?: string }) => {
      ...
      const handleCursorMoved = (data: {
        ...
        setParticipants(prev =>
          prev.map(participant => {
            if (
              (data.userId && participant.userId === data.userId) ||
  ...
  const updateElement = useCallback(
    (id: string, changes: Partial<AnyMathElement>) => {
      if (!canEdit) return
