...
export function useLiveClassWhiteboard(
  ...
  const applyStrokeOps = useCallback((prev: WhiteboardStroke[], ops: WhiteboardStrokeOp[]) => {
    ...
    ops.forEach(op => {
      ...
      if (
        !existing ||
        (incomingVersion >= currentVersion && JSON.stringify(existing) !== JSON.stringify(stroke))
      ) {
  ...
  const [studentWhiteboards, setStudentWhiteboards] = useState<Map<string, StudentWhiteboard>>(
    new Map()
  )
  ...
  const [remoteSelections, setRemoteSelections] = useState<
    Map<string, WhiteboardSelectionPresence>
  >(new Map())
  ...
  const [activeLayerId, setActiveLayerId] = useState<LayerConfig['id']>(
    role === 'tutor' ? 'tutor-broadcast' : 'student-personal'
  )
  const [assignmentOverlay, setAssignmentOverlay] = useState<
    'none' | 'graph-paper' | 'geometry-grid' | 'coordinate-plane' | 'chemistry-structure'
  >('none')
  ...
  const [aiRegionHints, setAiRegionHints] = useState<
    Array<{
      requestedBy: string
  ...
  const enqueueStrokeOps = useCallback(
  ...
  useEffect(() => {
    ...
    socket.on(
      'lcwb_student_whiteboard_state',
      (data: { studentId: string; strokes: WhiteboardStroke[] }) => {
    ...
    socket.on(
      ...
      (data: { studentId: string; ops: WhiteboardStrokeOp[] }) => {
        const maxSeq = Math.max(0, ...(data.ops || []).map(op => op._seq || 0))
    ...
    socket.on(
      'lcwb_student_visibility_changed',
      (data: { studentId: string; visibility: 'private' | 'tutor-only' | 'public' }) => {
        setStudentWhiteboards(prev => {
          const wb = prev.get(data.studentId)
          if (wb) {
    ...
    socket.on(
      'lcwb_public_student_stroke',
      (data: { studentId: string; stroke: WhiteboardStroke }) => {
        setStudentWhiteboards(prev => {
          const wb = prev.get(data.studentId)
          if (wb) {
            wb.strokes.push(data.stroke)
    ...
    socket.on(
      'lcwb_public_student_strokes_reset',
      (data: { studentId: string; strokes: WhiteboardStroke[] }) => {
        setStudentWhiteboards(prev => {
          const next = new Map(prev)
          const wb = next.get(data.studentId)
    ...
    socket.on(
      'lcwb_public_student_stroke_ops',
      (data: { studentId: string; ops: WhiteboardStrokeOp[] }) => {
        setStudentWhiteboards(prev => {
          const next = new Map(prev)
          const wb = next.get(data.studentId)
    ...
    socket.on(
      'lcwb_layer_config',
      (data: { visibility: Record<string, boolean>; lockedLayers: string[] }) => {
        setLayerConfig(prev =>
          prev.map(layer => ({
            ...layer,
    ...
    socket.on(
      'lcwb_export_attached',
      (data: { exportId: string; name: string; url: string; attachedBy: string }) => {
  ...
  const addTutorStroke = useCallback(
    (stroke: WhiteboardStroke) => {
      if (!socket || role !== 'tutor') return
  ...
  const changeVisibility = useCallback(
    (newVisibility: 'private' | 'tutor-only' | 'public') => {
  ...
  const viewStudentWhiteboard = useCallback(
    (studentId: string) => {
      if (!socket || role !== 'tutor') return
  ...
  const annotateOnStudentBoard = useCallback(
    (studentId: string, stroke: WhiteboardStroke) => {
      if (!socket || role !== 'tutor') return
  ...
  const updateCursor = useCallback(
    (x: number, y: number, pointerMode: 'cursor' | 'laser' = 'cursor') => {
  ...
  const updateSelectionPresence = useCallback(
    (strokeIds: string[], pageId?: string, color: string = '#2563eb') => {
      if (!socket || !roomId || !session?.user?.id) return
  ...
  const createBoardBranch = useCallback(
    (branchName: string) => {
  ...
  const switchBoardBranch = useCallback(
    (branchName: string) => {
      if (!socket || !roomId || !session?.user?.id || !branchName.trim()) return
  ...
  const setLayerLock = useCallback(
    (layerId: LayerConfig['id'], locked: boolean) => {
      setLayerConfig(prev => {
  ...
  const markSubmissionReviewed = useCallback(
    (studentId: string) => {
  ...
  const setDrawMuteForStudent = useCallback(
    (studentId: string, muted: boolean) => {
      if (!socket || role !== 'tutor' || !roomId) return
  ...
  const updateStrokeRateLimit = useCallback(
    (limit: number, windowMs: number = moderationState.strokeWindowMs) => {
      if (!socket || role !== 'tutor' || !roomId) return
      socket.emit('lcwb_tutor_moderation_update', {
        roomId,
  ...
  const replaceMyStrokes = useCallback(
    (nextOrUpdater: WhiteboardStroke[] | ((prev: WhiteboardStroke[]) => WhiteboardStroke[])) => {
      setMyStrokes(prev => {
        const next =
  ...
  const setAssignmentOverlayMode = useCallback(
    (
  ...
  const updateSpotlight = useCallback(
    (nextSpotlight: WhiteboardSpotlight) => {
  ...
  const requestAIRegionHint = useCallback(
    (region: { x: number; y: number; width: number; height: number }, context?: string) => {
  ...
  const promoteBreakoutBoard = useCallback(
    (breakoutRoomId: string, strokes: WhiteboardStroke[], sourceStudentId?: string) => {
      if (!socket || role !== 'tutor' || !roomId) return
  ...
  const exportAndAttachBoard = useCallback(
    (data: { format: 'png' | 'pdf'; fileName: string; dataUrl: string; studentId?: string }) => {
      if (!socket || role !== 'tutor' || !roomId) return
