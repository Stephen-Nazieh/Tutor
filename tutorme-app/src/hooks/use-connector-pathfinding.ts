...
export function useConnectorPathfinding(options: RoutingOptions = {}) {
   ...
   */
  const getStrokeRect = useCallback(
    (stroke: {
  ...
  const routeMultiple = useCallback(
    (connectors: Connector[], shapes: Map<string, ShapeNode>): Map<string, ConnectorPath> => {
   ...
   */
  const createStrokeFromConnector = useCallback((connector: Connector, path: ConnectorPath) => {
  ...
  const isPointNearConnector = useCallback(
    (point: Point, connectorPath: ConnectorPath, threshold: number = 5): boolean => {
  ...
  const findConnectorsNearPoint = useCallback(
    (point: Point, connectorPaths: Map<string, ConnectorPath>, threshold: number = 5): string[] => {
      const result: string[] = []
  ...
  const updateConnectorTarget = useCallback(
    (connectorId: string, newTargetId: string, newTargetPort?: Port): boolean => {
