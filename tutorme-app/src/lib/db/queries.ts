...
export async function getStudentDashboardData(studentId: string) {
  return cache.getOrSet(
    ...
    async () => {
      const [enrollments, bookingsWithClinic, progressRows, gamificationRow, achievements] =
        await Promise.all([
          drizzleDb
            .select()
...
export async function getAvailableClasses(
  ...
  const profiles =
    tutorIds.length > 0
      ? await drizzleDb.select().from(profile).where(inArray(profile.userId, tutorIds))
      : []
  const profileMap = new Map(profiles.map(p => [p.userId, p]))
    ...
    return {
      ...
      tutor: profileMap.get(c.tutorId)
        ? {
            id: c.tutorId,
            profile: {
              name: profileMap.get(c.tutorId)!.name,
              avatarUrl: profileMap.get(c.tutorId)!.avatarUrl,
              specialties: profileMap.get(c.tutorId)!.specialties,
            },
          }
        : null,
          ...
          student: userMap.get(b.studentId)
            ? {
                ...
                profile: studentProfileMap.get(b.studentId)
                  ? {
                      name: studentProfileMap.get(b.studentId)!.name,
                      avatarUrl: studentProfileMap.get(b.studentId)!.avatarUrl,
                    }
                  : null,
              }
            : null,
  ...
  return cache.getOrSet(
    ...
    async () => {
      ...
      return rows.map(r => ({
        ...
        user: {
          id: r.userId,
          profile: profileMap.get(r.userId)
            ? {
                name: profileMap.get(r.userId)!.name,
                avatarUrl: profileMap.get(r.userId)!.avatarUrl,
              }
            : null,
        },
      }))
...
export async function getContentLibrary(
  ...
  if (search) {
    const term = `%${search}%`
    conditions.push(
      sql`(${contentItem.title} ilike ${term} or coalesce(${contentItem.description}, '')::text ilike ${term})`
    )
  }
