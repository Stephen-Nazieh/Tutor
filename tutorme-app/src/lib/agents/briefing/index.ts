...
function parseSpotlightResponse(content: string): StudentSpotlight {
  ...
  return {
    ...
    watchOutFor:
      content
        .split('Watch Out For')[1]
        ?.split('Engagement Strategy')[0]
        ?.split('\n')
        .filter(l => l.trim().startsWith('-'))
        .map(l => l.replace('-', '').trim()) || [],
    engagementStrategy:
      content.split('Engagement Strategy')[1]?.split('Questions to Ask')[0]?.trim() || '',
    questionsToAsk:
      content
        .split('Questions to Ask')[1]
        ?.split('\n')
        .filter(l => l.trim().startsWith('-'))
        .map(l => l.replace('-', '').trim()) || [],
  }
 ...
 */
export async function generatePostClassReflection(sessionId: string): Promise<PostClassReflection> {
  const session = await getLiveSession(sessionId)
  if (!session) throw new Error('Session not found')
...
function parseReflectionResponse(content: string): PostClassReflection {
  return {
    ...
    highEngagementMoments:
      content
        .split('High Engagement')[1]
        ?.split('Low Engagement')[0]
        ?.split('\n')
        .filter(l => l.trim().startsWith('-'))
        .map(l => l.replace('-', '').trim()) || [],
    lowEngagementMoments:
      content
        .split('Low Engagement')[1]
        ?.split('Follow-Up')[0]
        ?.split('\n')
        .filter(l => l.trim().startsWith('-'))
        .map(l => l.replace('-', '').trim()) || [],
    followUpActions:
      content
        .split('Follow-Up')[1]
        ?.split('Growth Opportunities')[0]
        ?.split('\n')
        .filter(l => l.trim().startsWith('-'))
        .map(l => l.replace('-', '').trim()) || [],
    growthOpportunities:
      content
        .split('Growth Opportunities')[1]
        ?.split('\n')
        .filter(l => l.trim().startsWith('-'))
        .map(l => l.replace('-', '').trim()) || [],
  }
 ...
 */
export async function generateTeachingStrategies(sessionId: string): Promise<string[]> {
  const session = await getLiveSession(sessionId)
  if (!session) throw new Error('Session not found')
