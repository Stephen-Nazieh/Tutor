...
async function analyzeWithAI(
  ...
  try {
    ...
    const parsed = safeJsonParseWithSchema(result.content, engagementAnalysisSchema, {
      extract: true,
    })
 ...
 * Fast, no LLM call
 */
function analyzeWithRules(activities: StudentActivity[], sessionId: string): EngagementAnalysis {
  const scores = new Map<string, number>()
...
export async function detectConfusion(
  ...
  try {
    ...
    const parsed = safeJsonParseWithSchema(result.content, confusionDetectionSchema, {
      extract: true,
    })
 ...
 */
export async function suggestInterventions(sessionId: string): Promise<Intervention[]> {
  const session = await getLiveSession(sessionId)
  if (!session) throw new Error('Session not found')
  ...
  try {
    ...
    const parsed = safeJsonParseWithSchema(result.content, interventionListSchema, {
      extract: true,
    })
 ...
 */
export async function analyzeParticipationBalance(sessionId: string): Promise<{
  ...
  try {
    ...
    const parsed = safeJsonParseWithSchema(result.content, participationBalanceSchema, {
      extract: true,
    })
