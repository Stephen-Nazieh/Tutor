...
- "specSoFar" = a running snapshot, included on EVERY turn, of what the tutor has defined so far this conversation. Same keys as "spec" (all optional); include a field ONLY once the tutor has actually stated it, using their meaning in a short plain phrase, and OMIT anything not yet given (a nearly-empty {} early on is expected, growing as the chat proceeds). This is just a progress view for the tutor — it does NOT finalize anything and never puts a value into the saved "pci".`
