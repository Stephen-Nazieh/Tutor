...
export async function checkAndAwardBadges(
  ...
  if (
    ...
    stats.quizAverage >= 80
  )
    badgesToCheck.push('quiz_master')
  if (stats.sessionsJoined !== undefined && stats.sessionsJoined >= 5)
    badgesToCheck.push('social_butterfly')
  if (stats.messagesSent !== undefined && stats.messagesSent >= 50)
    badgesToCheck.push('active_participant')
...
export async function getBadgeStats(userId: string) {
  const [earnedRows, allBadges] = await Promise.all([
    drizzleDb
      .select({
        badgeId: userBadge.badgeId,
        bRarity: badge.rarity,
        bCategory: badge.category,
  ...
  return {
    ...
    completionPercentage: allBadges.length
      ? Math.round((earnedRows.length / allBadges.length) * 100)
      : 0,
    byRarity,
    byCategory,
