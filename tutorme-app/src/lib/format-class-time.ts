 ...
 */
export function formatClassTime(scheduledAt: string | Date, durationMinutes?: number): FormattedClassTime {
  ...
  if (ms < MS_HOUR) {
    ...
    return { formatted, timeRange, relative: minutes <= 1 ? 'Starting soon' : `Starts in ${minutes} minutes` }
  }
  if (ms < MS_DAY) {
    const hours = Math.round(ms / MS_HOUR)
    return { formatted, timeRange, relative: hours === 1 ? 'Starts in 1 hour' : `Starts in ${hours} hours` }
  }
  if (ms < 2 * MS_DAY) {
    return { formatted, timeRange, relative: `Tomorrow at ${date.toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit' })}` }
  }
  return { formatted, timeRange }
}
