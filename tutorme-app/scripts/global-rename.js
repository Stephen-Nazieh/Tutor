...
try {
  const output = execSync('find src -name "*curriculum*" -o -name "*Curriculum*"', { cwd: '/Users/nazy/ADK_WORKSPACE/TutorMekimi/tutorme-app', encoding: 'utf8' });
  const paths = output.trim().split('\n').filter(Boolean).sort((a, b) => b.length - a.length); // deepest first
